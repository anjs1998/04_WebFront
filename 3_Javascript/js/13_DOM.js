...
document.querySelector("#btn1").addEventListener("click", () => {
    ...
    // li1 부모 노드 찾아서 배경색 변경하기
     ...
     //( 요소. childNodes[index])
     // #test(ul) 의 자식노드중
     // 3번째 li태그를 찾아서 배경색 pink 지정하기(=9번 인덱스)
     ...
     // == test.childNodes[11]
     console.log( test.childNodes[9].nextSibling.nextSibling );//=11번 index
    ...
    // 또는 list[11].append("ZZZ");
     // 2. 마지막 자식으로 추가 ( append("내용" 또는 노드 또는 요소 n개) )
    list[11].append("12345","abcd","가나다라");

    // innerHTML은 DOM 리셋시 단점들이 있기 때문에 앞으로는 appendChild()를 쓸것.

    // 3. 첫번째 자식으로 추가(요소.prepend("내용" 또는 노드 요소 n개))
